...
@click.option("-s", "--species", required=True, type=int, multiple=True, help="Species to consider")
@click.option("-tr", "--traj_list", required=True, type=str, multiple=True, help="List of directories containing the trajectories")
...
def main(temperature, species, traj_list, goal_state_file, save_dir):
    ...
    for name in traj_list:
        print(name)
        with open(name + '/diffuse.json', 'r') as file:
            time = json.load(file)[0]

        Ntraj = len(time)
        for j in range(Ntraj):
            atomsj = io.read(name + '/XDATCAR'+str(j),index=':')
            for frame in range(len(atomsj)-1):
                ...
                SRO = np.triu(get_sro(atomsj[frame]))
                SRO_next = np.tri(get_sro(atomsj[frame+1]))
    ...
    with open(f'{save_dir}/dataset_{temperature}.json','w') as file:
